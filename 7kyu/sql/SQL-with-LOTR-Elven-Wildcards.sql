...
-- To aid the scribes, return the firstname and lastname column concatenated, separated by a space, into a single shortlist column, and 
-- capitalise the first letter of each name.


--  My Solution
SELECT CONCAT(UPPER(SUBSTRING(firstname, 1, 1)), LOWER(SUBSTRING(firstname FROM 2)), ' ', 
            UPPER(SUBSTRING(lastname, 1, 1)), LOWER(SUBSTRING(lastname FROM 2))) AS shortlist
FROM Elves
